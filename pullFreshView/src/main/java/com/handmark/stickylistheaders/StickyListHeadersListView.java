package com.handmark.stickylistheaders;
	...
	private void scrollChanged(int reportedFirstVisibleItem) {
		...
		if (childCount != 0) {
			...
			for (int i = 0; i < childCount; i++) {
				...
				if (viewToWatch == null
						|| (!viewToWatchIsFooter && !(child instanceof WrapperView ?((WrapperView) viewToWatch).hasHeader() :false))
						|| ((childIsFooter || child instanceof WrapperView ?((WrapperView) child).hasHeader() : false) && childDistance < watchingChildDistance)) {
					viewToWatch = child;
			...
			if (viewToWatch != null && viewToWatch instanceof WrapperView 
