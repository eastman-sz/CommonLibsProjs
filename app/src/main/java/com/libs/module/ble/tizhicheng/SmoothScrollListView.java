...
//				this.smoothScrollToPositionFromTop(currentItemPosition + 1, 0, scrollDuration);
    ...
    public interface onScrollItemListener{
        void onScroll(int postion);
    }

    onScrollItemListener onScrollItemListener = null;

    public void setOnScrollItemListener(SmoothScrollListView.onScrollItemListener onScrollItemListener) {
        this.onScrollItemListener = onScrollItemListener;
    }

    private void onScrollItemChg(int position){
        if (null == onScrollItemListener){
            return;
        }
        onScrollItemListener.onScroll(position);
    }
}
